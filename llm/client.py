...
class OAIClient(LLMClient):
    ...
    Responsibility:
    - translate normalized LLMRequest into OpenAI payload
    - execute SDK request
    - normalize SDK response into application-level DTO
    """

    def __init__(self) -> None:
        """Initialize OpenAI SDK client using configured settings."""
        self._llm_config = load_llm_settings()

        kwargs: dict[str, Any] = {"api_key": self._llm_config.api_key}
    ...
    def create(
        ...
        *,
        ...
        """
        Execute one Responses API request.

        When `previous_response_id` is provided, the request is chained into
    ...
    def _build_payload(
        ...
        *,
        ...
        """Build OpenAI Responses API payload from normalized request."""
        ...
        if getattr(request, "prompt_cache_key", None) is not None:
    ...
    def _to_llm_response(response: Any) -> LLMResponse:
        """
        Normalize OpenAI Responses API SDK object into application-level LLMResponse.

        For responses.parse(), parsed output is the source of truth.
        output_text is diagnostic/display text only and may be empty for structured outputs.

        Token metrics are printed compactly for runtime cost visibility:
        - input tokens
        - cached input tokens
        ...
        def _get_detail_value(container: Any, key: str) -> int:
            """
            Read detail values from either SDK objects or dicts.
            """
        ...
        def _print_usage_metrics(resp_id: str | None, usg: dict[str, Any] | None) -> None:
            if usg is None:
                print(f"[llm_usage] response_id={resp_id} usage=None")
                return
